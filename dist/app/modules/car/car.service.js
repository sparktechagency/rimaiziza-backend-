...
const updateCarByIdToDB = (carId, payload) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (payload.arrayAction) {
        ...
        // -------------------------- Facilities --------------------------
        if (field === "facilities") {
            const isFacilityPayload = (val) => typeof val === "object" &&
                val !== null &&
                "label" in val &&
                "value" in val;
        ...
        const updated = yield car_model_1.Car.findOneAndUpdate({ _id: carId }, updateQuery, {
            new: true,
        });
        if (!updated) {
    ...
    const updated = yield car_model_1.Car.findOneAndUpdate({ _id: carId }, cleanPayload, {
        new: true,
    });
    if (!updated) {
        throw new ApiErrors_1.default(404, "Car not found or not owned by user");
...
const parseQueryParams = (params) => {
    const { lat, lng, maxDistanceKm, limit, country, state, city, minPrice, maxPrice, searchTerm, brand, transmission, fuelType, seatNumber, minYear, maxYear, facilities, availableDays, sortBy = "price", sortOrder = "asc", userId, pickupDateTime, returnDateTime, } = params;
    ...
    return {
        ...
        sortBy: sortBy,
        sortOrder: sortOrder,
        pickupDateTime: typeof pickupDateTime === "string" ? pickupDateTime : undefined,
        returnDateTime: typeof returnDateTime === "string" ? returnDateTime : undefined,
    };
};
// Main service function
const getNearbyCarsFromDB = (params) => __awaiter(void 0, void 0, void 0, function* () {
    const parsedParams = parseQueryParams(params);
    const { lat, lng, userId, maxDistanceKm, limit, country, state, city, minPrice, maxPrice, searchTerm, brand, transmission, fuelType, seatNumber, minYear, maxYear, facilities, availableDays, sortBy, sortOrder, pickupDateTime, returnDateTime, } = parsedParams;
    ...
    if (pickupDateTime && returnDateTime) {
        ...
        if (isNaN(from.getTime()) || isNaN(to.getTime())) {
            throw new ApiErrors_1.default(400, "Invalid pickup or return datetime");
        }
        if (from >= to) {
            throw new ApiErrors_1.default(400, "Return datetime must be after pickup datetime");
        }
        const availabilityFlags = yield Promise.all(cars.map((car) => __awaiter(void 0, void 0, void 0, function* () {
            try {
                yield (0, car_utils_1.validateAvailabilityStrict)(car._id.toString(), from, to);
                return true;
            }
            catch (_a) {
                return false;
    ...
    // -------------------- TRIPS --------------------
    const tripCountMap = yield (0, car_utils_1.getCarTripCountMap)(filteredCars.map((c) => c._id));
    ...
    if (userId && filteredCars.length > 0) {
        ...
        const favorites = yield favoriteCar_model_1.FavoriteCar.find({
            userId,
    ...
    let paidCarIds = new Set();
    if (userId && filteredCars.length > 0) {
        const carIds = filteredCars.map((c) => c._id);
        // Find all paid bookings for this user across these cars
        const paidBookings = yield booking_model_1.Booking.find({
            carId: { $in: carIds },
        ...
        for (const booking of paidBookings) {
            ...
            const isPaid = (transaction === null || transaction === void 0 ? void 0 : transaction.status) === transaction_interface_1.TRANSACTION_STATUS.SUCCESS;
            if (isPaid) {
                paidCarIds.add(booking.carId.toString());
            }
        }
    }
...
const getCarByIdForUserFromDB = (id, userId) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (car.assignedHosts) {
        ...
        totalBookings = yield booking_model_1.Booking.countDocuments({
    ...
    const hasUserPaid = yield (0, car_utils_1.checkIfUserHasPaid)(id, userId);
    return Object.assign(Object.assign({}, car.toObject()), { hasUserPaid, trips: trips || 0, isAvailable,
...
const getCarsByHostFromDB = (hostId) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!car) {
        throw new ApiErrors_1.default(200, "No active car found for this host");
    }
    // -------------------- FAVORITE --------------------
    const isBookmarked = yield favoriteCar_model_1.FavoriteCar.exists({
        userId: objectHostId,
    ...
    const availabilityCalendar = yield (0, car_utils_1.getCarCalendar)(car._id.toString());
    // -------------------- TRIPS --------------------
    const tripCount = yield (0, car_utils_1.getCarTripCount)(car._id);
    // (car as any).trips = tripCountMap[car._id.toString()] ?? 0;
    car.trips = tripCount !== null && tripCount !== void 0 ? tripCount : 0;
    // -------------------- REVIEWS --------------------
    ...
    car.starCounts = (_c = reviewSummary === null || reviewSummary === void 0 ? void 0 : reviewSummary.starCounts) !== null && _c !== void 0 ? _c : {
        1: 0,
        2: 0,
        3: 0,
        4: 0,
    ...
    car.hostTotalEarning = (_f = (_e = earningResult[0]) === null || _e === void 0 ? void 0 : _e.totalEarning) !== null && _f !== void 0 ? _f : 0;
    return Object.assign(Object.assign({}, car), { availabilityCalendar });
});
exports.CarServices = {
    createCarToDB,
    getAllCarsFromDB,
