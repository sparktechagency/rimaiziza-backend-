        ...
        { $unwind: { path: "$transaction", preserveNullAndEmptyArrays: true } },
        {
            $group: {
                ...
                totalBookings: {
                    $sum: {
                        $cond: [
                            { $and: [{ $ne: ["$bookingStatus", booking_interface_1.BOOKING_STATUS.CANCELLED] }, { $eq: ["$transaction.status", transaction_interface_1.TRANSACTION_STATUS.SUCCESS] }] },
                            1,
                            0,
                        ],
                    },
                ...
                cancelledBookings: {
                    $sum: {
                        $cond: [{ $eq: ["$bookingStatus", booking_interface_1.BOOKING_STATUS.CANCELLED] }, 1, 0],
                    },
                },
            },
        },
